...
def send_reminder():
    ...
    body_html = f"""<div style="font-family:Georgia,serif;max-width:500px;margin:0 auto;padding:24px;background:#f5f0e8">
  ...
  <p style="font-size:15px;line-height:1.7;color:#1a1410;margin-bottom:16px">Hey guys - time to run the <strong>{month_str} Outstanding Balance Report</strong>. Please drop the following 3 files in the chat:</p>
  <div style="background:#ede8e0;border-left:4px solid #1F4E79;padding:16px 20px;margin-bottom:20px;font-family:sans-serif">
    <div style="font-size:13px;color:#1a1410;line-height:2">
      <div>1. Outstanding Balances CSV</div><div>2. Leases / Rent Roll CSV</div><div>3. Last Month Report - the {prev_month} Excel file</div>
    </div>
  </div>
  ...
  <div style="margin-top:24px;padding-top:16px;border-top:1px solid #d8d0c0;font-size:11px;color:#c8c0b0;text-align:center">Alyssa Gray Realty - {month_str}</div>
    ...
    print(f"Reminder sent for {month_str}")
